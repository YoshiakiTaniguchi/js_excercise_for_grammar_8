...
while (input !== commands[3]) {
    input = prompt("「確認,追加,削除,終了」以外は入力できません")
    if (input === commands[0]) {
        showTodos();
...
if (input !== commands) {
    cosole.log("「確認,追加,削除,終了」以外は入力できません");
};
...
function deeteTodo() {
    ...
    } else if (num === 0, 1, 2, 3, 4) {
        const deletedTodos = todos.splice(num);
